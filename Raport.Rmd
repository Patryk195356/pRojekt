...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# 1. Tworzymy tabelę kontyngencji i wykonujemy chi-kwadrat
table_chi <- table(dane$Gender, dane$`Passive.Smoker`)
chi_result <- chisq.test(table_chi)

# zapisujemy wynik p-value jako tekst do wykresu
p_text <- paste0("Chi-sq p = ", signif(chi_result$p.value, 3))

# 2. Przygotowujemy dane do wykresu
plot_data <- dane %>%
...
# 3. Tworzymy wykres słupkowy z procentami i adnotacją p-value
