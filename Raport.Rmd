...
dane %>%
  ...
  labs(
    title = "Wiek zachorowania na nowotwór płuca wg płci",
    subtitle = "Średnia ± SD; test t-Studenta",
    x = "Płeć",
    y = "Wiek (lata)"
  ) +
...
# Tworzymy nową kolumnę Level_num automatycznie
# Każdy unikalny poziom Level (tekst lub factor) zostanie zamieniony na liczbę
dane_clean <- dane %>%
  mutate(
    Level_num = as.numeric(factor(Level))  # automatyczne mapowanie wszystkich unikalnych wartości
  ) %>%
  filter(
    !is.na(Age),         # usuwa brakujące wieki
    !is.na(Level_num)    # usuwa brakujące poziomy choroby
  )

# Sprawdzamy, czy zostały wiersze
...
ggplot(plot_data, aes(x = Gender, y = percent, fill = Passive.Smoker)) +
  ...
  labs(title = "Rozkład biernego palenia w zależności od płci",
       subtitle = p_text,
       x = "Płeć",
       y = "Procent",
       fill = "Passive.Smoker") +
